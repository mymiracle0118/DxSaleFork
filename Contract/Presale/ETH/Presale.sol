...
//   }
  ...
  function ownerWithdrawTokens () private onlyPresaleOwner {
    ...
    TransferHelper.safeTransfer(address(presale_info.sale_token), presale_info.presale_owner, IERC20(presale_info.sale_token).balanceOf(address(this)));
  ...
  function forceFailIfPairExists () public {
    ...
    if (presale_lock_forwarder.uniswapPairIsInitialised(address(presale_info.sale_token), address(WETH))) {
  ...
  function addLiquidity() public nonReentrant onlyPresaleOwner {
    ...
    // burn unsold tokens
    uint256 remainingSBalance = IERC20(presale_info.sale_token).balanceOf(address(this));
